...
namespace ServiceFabricQueuedServices.Services
{
    public static class QueuedServiceInstanceListenerUtility
    {
			...
			Func<string> queueNameProvider = null)
		{
			...
			{
				string listenConnectionString = context.CodePackageActivationContext.GetConfigurationPackageObject(configPackageName).Settings.Sections[sectionName].Parameters[listenConnectionStringParameterName].Value;
